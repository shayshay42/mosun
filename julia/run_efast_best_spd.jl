...
function salib_fast_sample(
    ...
    for i in 1:D
        ...
        phi = two_pi * rand(rng)
        for j in 1:D
            g = @. 0.5 + (1.0 / pi) * asin(sin(omega2[j] * s + phi))
            X[(z0 + 1):(z0 + N), j] = g
        end
    end
...
function main()
    ...
    println("  horizon_days=$horizon_days, save_dt=$save_dt, threads=$(Threads.nthreads())")
    ...
    Threads.@threads for i in 1:n_eval
        ...
        if done % 25 == 0 || done == n_eval
            lock(progress_lock) do
                println("  progress $done / $n_eval")
            end
        end
    end
    ...
    failures = count(x -> x != "ok", status[1:n_eval])
    ...
    meta = Dict(
        ...
        "output" => "best_spd_pct = min_t 100*(Btumor(t)/Btumor(0)-1), t in (0,horizon]",
